...
If the injected memory snippets above are not enough to answer the user's question, you may proactively call the following tools to retrieve more information:

- **tdai_memory_search**: Search structured memories (L1). Best for recalling key information such as user preferences, historical event nodes, and rules.
- **tdai_conversation_search**: Search raw conversations (L0). Best for locating exact message wording, timelines, and contextual details. It can also supplement or verify the results of memory_search.
- **read_file** (paths from Scene Navigation): Use this after locating the relevant scene when you need the full scene-level persona, event flow, or stage conclusions.

### ⚠️ Call Limit
Within a single conversation turn, **tdai_memory_search and tdai_conversation_search may be called at most 3 times in total**.
- If the first search returns no result, you may retry with different keywords or a different tool, but the total number of calls must still stay within 3.
- If you still get no result after 3 searches, that means the information is not present in memory. Respond directly using the information you already have, and do not keep searching.
</memory-tools-guide>`
...
async function performAutoRecallInner(params: {
  ...
  if (memoryLines.length > 0) {
    prependContext =
      `<relevant-memories>\nBelow are the relevant memories recalled for the current conversation. They do not represent the current task progress and are provided only as reference:\n\n${memoryLines.join("\n")}\n</relevant-memories>`;
  }
 ...
 *   - activity_start_time / activity_end_time (time range): activity time range, e.g. "trip from 2025-05-01 to 2025-05-10"
 *   - All three time fields may be empty/undefined — handled gracefully.
 *
 * Output examples:
 *   - [persona] The user's name is Wang Xiaoming, age 30, and they are a software engineer.
 *   - [episodic|Travel Plans] The user plans to travel to Japan in May. (Activity time: 2025-05-01 ~ 2025-05-10)
 *   - [episodic] The user worked overtime very late today. (Activity time: 2025-03-01)
 *   - [instruction] The user wants answers in Chinese and prefers them concise.
 */
interface FormatableMemory {
  type: string;
  content: string;
  scene_name?: string;
  /** Activity time range start, may be empty */
  activity_start_time?: string;
  /** Activity time range end, may be empty */
  activity_end_time?: string;
  /** Activity point-in-time, may be empty */
  timestamp?: string;
}
...
function formatMemoryLine(m: FormatableMemory): string {
  ...
  if (start && end) {
    // Time range: both start and end
    line += ` (Activity time: ${start} ~ ${end})`;
  } else if (start) {
    // Time range: only start
