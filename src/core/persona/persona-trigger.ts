  ...
  async shouldGenerate(): Promise<TriggerResult> {
    ...
    if (cp.request_persona_update) {
      const result: TriggerResult = {
        should: true,
        reason: `Explicit request: ${cp.persona_update_reason || "Agent requested an update"}`,
      };
    ...
    if (
      ...
      const result: TriggerResult = { should: true, reason: "Cold start: first extraction completed and scene files exist" };
    ...
    if (
      ...
      const result: TriggerResult = { should: true, reason: "Recovery: persona.md body is missing or empty and must be regenerated" };
