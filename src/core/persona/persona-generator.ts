  ...
  async generateLocalPersona(triggerReason?: string): Promise<boolean> {
    ...
    if (changedSceneContents.length > 0) {
      changedScenesContent =
        `\n\n## 📄 Full Contents of Changed Scenes\n\n` +
        `*Since the last Persona update, the following ${changedSceneContents.length} scenes have changed. The system has preloaded their full contents for you:*\n\n` +
        changedSceneContents.join("\n\n") +
        `\n\n---\n\n` +
        `⚠️ **Focus on the changed scenes**: the scenes above contain the **newly added / modified content** since the last update, so analyze the new information in them carefully.\n`;
    } else {
      changedScenesContent = `\n\n⚠️ **No changed scenes**: all scenes were already analyzed in the last Persona update, so this run may review all scenes directly from a global perspective.\n`;
    }
