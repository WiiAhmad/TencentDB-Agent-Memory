...
export class LocalMemoryCleaner {
  ...
  async runOnce(nowMs = Date.now()): Promise<void> {
    ...
    // Compute the cutoff using a local-calendar-day retention policy.
  ...
  private async cleanDirectory(dirPath: string, cutoffMs: number): Promise<CleanupStats> {
    ...
    for (const entry of entries) {
      ...
      // Only date-sharded files are supported: YYYY-MM-DD(.jsonl/.json)
...
function computeCutoffMsByLocalDay(nowMs: number, retentionDays: number): number {
  // Calendar-day policy: keep today plus the previous retentionDays-1 days.
  // The deletion threshold is 00:00:00.000 on keepStart in the local timezone.
  const now = new Date(nowMs);
