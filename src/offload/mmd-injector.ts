...
export async function injectMmdIntoMessages(
  ...
  traceOffloadDecision({
    ...
    output: {
      result: `Injected MMD messages:${totalMmdTokens} tokens (active only)`,
...
async function buildActiveMmdBlock(
  ...
  try {
    ...
    return [
      ...
      `【Current active task Mermaid flowchart】This is a progress snapshot for the task you have been working on recently. Tool uses below this message are not summarized yet, and the process may lag; use it for reference only.`,
      taskGoal ? `**Task goal:** ${taskGoal}` : "",
      `**Task file:** ${activeMmdFile}`,
      nodeIds.length > 0
        ? `**Node index:** Use node_id to find the corresponding tool-call record in the agent's offload-*.jsonl files. To inspect the raw tool call and full result for a node, find that entry's result_ref and read the referenced file.`
        : "",
      "```mermaid",
      mmdContent,
      "```",
      `Nodes marked "doing" are the recent focus (note: this may lag, and tool uses below this message are not counted yet; reference only). Nodes marked "done" are complete. Use this to stay oriented and avoid repeating completed work.`,
      `</current_task_context>`,
    ]
      .filter((line) => line !== "")
      .join("\n");
