...
function buildHistoryMmdText(filename: string, mmdContent: string): string {
  ...
  return [
    ...
    `【Historical task context】Below is the state diagram for a completed or paused historical task.`,
...
function buildHistoryMmdMetaText(filename: string, mmdContent: string): string {
  ...
  return [
    ...
    `【Historical task summary】Below is metadata for a historical task; the original diagram was omitted to save context.`,
    taskGoal ? `**Task goal:** ${taskGoal}` : "",
    `**Task file:** ${filename}`,
