...
| `--tcvdb-url` | Yes | — | TCVDB service endpoint |
...
| `--rewrite-manifest` | No | `true` | Rewrite manifest.json for tcvdb |
...
├── tsconfig.json         # Migration script TypeScript build config
...
The migration script imports storage implementations from `../../src/` (such as `VectorStore` and `TcvdbMemoryStore`), but it **does not depend on `openclaw/plugin-sdk`**. Config write-back uses `json5` directly.
